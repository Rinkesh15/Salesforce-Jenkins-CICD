pipeline {
    ...
    stages {
        ...
        /*--------------------------------------------------
         AUTH TARGET ORG (ONLY TARGET, NO SOURCE NEEDED)
        --------------------------------------------------*/
    ...
    post {
        success {
            echo "🚀 SUCCESS — CICDJenkins.cls deployed to target org!"
        }
        failure {
            echo "❌ FAILED — Deployment error. Check logs!"
        }
    }
}
